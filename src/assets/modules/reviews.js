...
function listing(page = 1, append = true) {
  ...
  fetch(ApiUrl("nextash_store.events.review.listing/?" + query))
    .then((res) => res.json())
    .then((data) => {
      data = data.message;
      if (data.count == 0) {
...
on("submit", "#createReview", function (e) {
  ...
  if (!description || description == "") {
    ...
    fetch(ApiUrl("nextash_store.events.review.create"), {
      method: "POST",
      headers: new Headers({
...
on("click", ".deleteReview", function (e) {
  ...
  fetch(ApiUrl("nextash_store.events.review.delete"), {
    method: "POST",
    headers: new Headers({
      "Content-Type": "application/json",
      "X-Frappe-CSRF-Token": csrf_token(),
...
on("submit", "#updateReview", function (e) {
  ...
  if (!description.value || description.value == "") {
    ...
    fetch(ApiUrl("nextash_store.events.review.update"), {
      method: "POST",
      headers: new Headers({
        "Content-Type": "application/json",
        "X-Frappe-CSRF-Token": csrf_token(),
